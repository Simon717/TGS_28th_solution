...
def do_resize_3_channels(image, H, W):
    out = np.zeros((3, H, W))
    for i in range(3):
        out[i] = do_resize(image[i], H, W)
    return out

def do_center_pad_3_channels(image, PAD):
    out = np.zeros((3, IMG_NET_SIZE, IMG_NET_SIZE))
    for i in range(3):
        out[i] = do_center_pad(image[i], PAD)
    return out
